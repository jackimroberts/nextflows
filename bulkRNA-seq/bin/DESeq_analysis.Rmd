...
pacman::p_load(tidyverse,DESeq2,pheatmap,biomaRt,ggrepel,
...
mart <- mart <- useMart(biomart = "ensembl", dataset = "mmusculus_gene_ensembl")
...
print(p)
ggsave(file.path(save_dir, "plots", "top_genes_heatmap.png"), plot = p)
...
for (pair in condition_pairs) {
  ...
  for (collection_name in available_collections) {
    ...
    # run fgsea
    fgsea_res <- suppressMessages(fgsea::fgsea(pathways = msig_sets[[collection_name]], 
                              stats = gene_list,
                              minSize = 15,
                              maxSize = 500,
                              nproc = 1)) %>% 
