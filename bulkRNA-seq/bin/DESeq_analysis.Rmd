...
pacman::p_load(tidyverse,DESeq2,pheatmap,biomaRt,
               RColorBrewer,knitr,clusterProfiler,msigdbr,fgsea,openxlsx)
...
# process each tuple (sample) from the collected channel
for (sample_tuple in collected_counts) {
  # extract metadata from first element of tuple
  meta <- sample_tuple[[1]]
  
  # create metadata row with id, name, condition, and any additional columns
  meta_row <- tibble(
    id = meta[[1]],
    name = meta[[2]],
    condition = meta[[3]]
  ...
  if (length(meta) > 3) {
    meta_row$extra <- meta[[4]]
  }
  
  # append to metadata table
  metadata <- bind_rows(metadata, meta_row)
  
  # read count file (second element of tuple) and extract gene IDs and counts
  sample_counts <- read_tsv(sample_tuple[[2]], skip = 1) %>%
    select(Geneid, count = last_col())
  
  # if this is the first sample, initialize counts table
  if (nrow(counts) == 0) {
    counts <- sample_counts %>%
      rename(!!meta$name := count)
  } else {
    # append counts as new column
    counts <- counts %>%
      left_join(sample_counts %>% rename(!!meta$name := count), by = "Geneid")
  }
}
# separate extra column by tabs if it exists
if ("extra" %in% colnames(metadata)) {
  metadata <- separate(metadata, extra, sep = "\t", fill = "right")
}
...
```{r gene_names}
# retrieve gene annotations from Ensembl using biomaRt
mart <- useEnsembl(biomart = "ensembl", 
                   dataset = "mmusculus_gene_ensembl",  # mouse genome
                   mirror = "asia")  # use asia mirror for better connectivity
...
cat("Retrieved gene symbols for", nrow(g), "out of", nrow(counts), "genes\n")
...
# data for 20 most highly expressed transcrips
...
pheatmap(heatmap_data, 
         cluster_rows = FALSE,
         main = "Top 20 Most Highly Expressed Genes",
         filename = file.path(save_dir, "plots", "top_genes_heatmap.png"))
...
pheatmap(sampleDistMatrix,
         ...
         filename = file.path(save_dir, "plots", "sample_distances.png"))
...
all_msig <- msigdbr(species = "Mus musculus")

# create nested list: collections -> pathways -> genes
msig_sets <- all_msig %>%
  split(.$gs_subcat) %>%  # split by subcategory first
...
condition_pairs <- combn(unique(metadata$condition), 2, simplify = FALSE)
...
for (pair in condition_pairs) {
  ...
  volcano_plot <- ggplot(res, aes(log2FoldChange, -log10(padj + 1e-300))) +
    ...
    geom_text_repel(data = slice_max(
                            slice_min(sig_genes, padj, n = 20),
                          abs(log2FoldChang),n=20), 
  ...
  selected_collections <- c("HALLMARK", "CP_KEGG", "CP_REACTOME", "GO_BP")
  ...
  for (collection_name in available_collections) {
    ...
    # run fgsea
    fgsea_res <- fgsea::fgsea(pathways = msig_sets[[collection_name]], 
                              stats = gene_list,
                              minSize = 15,
                              maxSize = 500,
                              nproc = 1) %>% 
